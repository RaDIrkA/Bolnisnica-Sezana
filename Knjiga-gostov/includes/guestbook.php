...
$ERROR_MSG_FLOOD_DETECTED = "Skušate objavljati prepogosto.";
$READ_ONLY_MODE = FALSE;
$MAX_WORD_LENGTH = 125;
$ERROR_MSG_MAX_WORD_LENGTH = "Želeli ste objaviti besedo ki je predolga.";
$MIN_POST_DELAY = 5;
$MAX_POST_DELAY = 7200;
$ERROR_MSG_MIN_DELAY_STRING = "Poskušali ste prehitro objaviti.";
$ERROR_MSG_MAX_DELAY_STRING = "Predolgo ste čakali z objavo.";
...
function guestbook_entries_action($idArray, $banip = FALSE, $action) {
 	...
 	if(guestbook_open_for_read() === FALSE) {	// Acquires shared lock on guestbook file
 		die("Ne morem odpreti dadtoteko za branje knjigo gostov.");
 	}
 	$raw_entries = @file(guestbook_file_path());
 	guestbook_close();	// Releases shared lock 
 	if($raw_entries === FALSE) {
 		die("Ne morem dobiti vnosov iz knjige gostov.");
 	}
	...
	if(guestbook_open_for_writing() === FALSE) {
		die("Ne morem odpreti datoteko za pisanje v knjigo gostov."); 
	}
	...
	global $ERROR_MSG_BAD_WORD;
 	...
 	if($ENABLE_COMMENT_FIELD === TRUE) {
 		...
 		if(isset($entry["comments"])) {
 			die("Polje za komentarje je onemogočeno.");
 		}
 	}
	...
	global $READ_ONLY_MODE;
 	...
 	if(guestbook_validate($entry)) {
 		...
 		if($READ_ONLY_MODE === TRUE) {
 			$dbs_error = htmlspecialchars_default("Knjiga gostov je samo v načinu za branje.");
 			return FALSE;
 		}
	 	...
	 	if(!file_exists(guestbook_file_path())) {
	 		if(touch(guestbook_file_path()) === FALSE) {
	 			$dbs_error = htmlspecialchars_default("Ne morem mape podatkov knjige gostov.");
	 			return FALSE;
	 		}
	 	}
 		...
 		if(guestbook_open_for_read() === FALSE) {	// Acquires shared lock on guestbook file
 			$dbs_error = htmlspecialchars_default("Ne morem odpreti datoteke za branje knjige gostov.");
 			return FALSE;
	 	}
	 	$oldContents = @file_get_contents(guestbook_file_path());
	 	guestbook_close();	// Releases shared lock
	 	if($oldContents === FALSE) {
			$dbs_error = htmlspecialchars_default("Ne morem dobiti vsebino datoteke knjige gostov."); 
			return FALSE;
	 	}
		...
		if(guestbook_open_for_writing() === FALSE) {
			$dbs_error = htmlspecialchars_default("Ne morem odpreti datoteke za pisanje knjige gostov."); 
			return FALSE;
		}
		...
		// Write new entry
	 	...
	 	if(isset($ADMIN_EMAIL_ADDRESS) && !empty($ADMIN_EMAIL_ADDRESS)) {
			if(mail($ADMIN_EMAIL_ADDRESS,
				($approved?"":"PROSIM UREJAJ: ") . 
				"V vašo knjigo gostov se je podpisal/a " . 
				value_or_blank($entry_stripped, 'name'),
				($approved?"":"Morate odobriti ali izbrisati ta vnos v knjigi gostov:\n") .
				"Ime: " . value_or_blank($entry_stripped, 'name') . "\n" .
				...
				$dbs_error = htmlspecialchars_default("Ne morem poslati obvestila.");
				return FALSE;
			}
	 	}
...
function ban_add($ipaddress) {
 	...
 	if($ban_fp === FALSE) {
 		die("Ne morem odpreti BAN datoteke za pisanje");
 	}
...
function unban($ipAddressArray) {
	...
	if($bans !== FALSE) {
		...
		// Remove ip addresses from ban list
	 	foreach ($ipAddressArray as $ipaddress) {
			...
			if($idx !== FALSE) {
				...
				die("Določen je bil neveljaven IP .");
			}
		...
		// Rewrite data to file 	
	 	...
	 	if($ban_fp === FALSE) {
	 		die("Ne morem odpreti datoteke ta pisanje.");
	 	}
		@flock($ban_fp, LOCK_EX);
		...
		die("Ne morem dobiti seznama trenutnih BAN-ov.");
	}
...
function bad_word_add($word) {
	...
	if(is_bad_word($word)) {
 	...
 	if($bad_word_fp === FALSE) {
 		die("Ne morem odpreti datoteke za pisanje z grdo besedo ");
 	}
...
function remove_bad_word($wordArray) {
	...
	if($bad_words !== FALSE) {
		...
		die("Ne morem dobiti seznama grdih besed.");
	}
